package com.misys.tiplus2.apps.ti.kernel.extpm.entity.persister;

import java.sql.PreparedStatement;
import java.sql.ResultSet;
import java.sql.SQLException;

import com.misys.tiplus2.persistent.EntityPersister;
import com.misys.tiplus2.apps.ti.kernel.extpm.entity.ExtEvent;
import com.misys.tiplus2.apps.ti.kernel.extpm.entity.Key_ExtEvent;
import com.misys.tiplus2.apps.ti.kernel.event.entity.Key_event;
import com.misys.tiplus2.apps.ti.kernel.event.entity.event;
import com.misys.tiplus2.enigma.customisation.entity.EnigmaCustomisedEntity.ExtraDataMap;

@SuppressWarnings("unchecked")
public class ExtEventPersister extends EntityPersister {

	// This code will be generated by the builder 
	
	public ExtEventPersister() {
		setKeyMembers(new MemberDescriptor[] {
            new MemberDescriptor("key", "EXTEVENT.KEY29", getBinderFactory().getBinderForType("Long")),
		});
			
		setMembers(new MemberDescriptor[] {
            new MemberDescriptor("typeFlag", "EXTEVENT.TYPEFLAG", getBinderFactory().getBinderForType("Integer")),
            new MemberDescriptor("versionStamp", "EXTEVENT.TSTAMP", getBinderFactory().getBinderForType("Integer")),
            new MemberDescriptor("Initialised", "EXTEVENT.INITED", getBinderFactory().getBinderForType("Integer")),
            new MemberDescriptor("event.Key", "EXTEVENT.EVENT", getBinderFactory().getBinderForType("Long"), event.class),
            new MemberDescriptor("extraDataMap.ExtFixed", "EXTEVENT.AGNTWAMT", getBinderFactory().getBinderForType("Double")),
            new MemberDescriptor("extraDataMap.ExtFixed", "EXTEVENT.AGNTAMT", getBinderFactory().getBinderForType("Double")),
		});

		setForeignKeyMappings(new ForeignKeyMappingDescriptor[] {
			new ForeignKeyMappingDescriptor("event", event.class),
		});
	}

    private static final String tableName = "EXTEVENT";

    /**
     * Gets the table name - in this case EXTEVENT
     * <p>
     * Note that the <i>schema</i> must be prefixed to this when constructing
     * SQL statements, for example: <code>schema.EXTEVENT</code>
     * 
     * @return the table name
     */
	@Override
	public String getTableName() {       
		return tableName;
	}

	@Override
	public Class<?> getEntityClass() {
		return ExtEvent.class;
	}

	public Class<?> getKeyClass() {
		return Key_ExtEvent.class;
	}

	@Override
	public ExtEvent constructEntity(final Object[] keyValues, final Object[] resultRow) {
		final ExtEvent entity = new ExtEvent();
		
		// Key members
		final Key_ExtEvent key = entity.get$key();
		assignKeyValues(key, keyValues);

		assignMemberValues(entity, resultRow);
		
		return entity;
	}
	
	@Override
	public Key_ExtEvent constructKey(final Object[] keyValues) {
		final Key_ExtEvent key = new Key_ExtEvent();
		assignKeyValues(key, keyValues);
		return key;
	}
	
	@Override
	public void assignKeyValues(final Object object, final Object[] resultRow) {
		final Key_ExtEvent result = (Key_ExtEvent)object;
		int index = 0;
		result.getKey().setDbValue((java.lang.Long)resultRow[index++]);
	}
	
	@Override
	public void assignMemberValues(final Object object, final Object[] resultRow) {
		final ExtEvent result = (ExtEvent)object;
		int index = 0;
		result.setTypeFlag((java.lang.Integer)resultRow[index++]);
		result.setVersionStamp((java.lang.Integer)resultRow[index++]);
		result.getInitialised().setDbValue((java.lang.Integer)resultRow[index++]);
		result.getEvent().getKey().setDbValue((java.lang.Long)resultRow[index++]);
		result.getExtraDataMap().getExtFixed("AGNTWAMT").setDbValue((java.lang.Double)resultRow[index++]);
		result.getExtraDataMap().getExtFixed("AGNTAMT").setDbValue((java.lang.Double)resultRow[index++]);
	}

	@Override
	public Object[] getKeyMemberValues(final Object object) {
		final Key_ExtEvent entity = (Key_ExtEvent)object;
		final Object[] result = new Object[1];
		int index = 0;
		result[index++] = entity.getKey().getDbValue();
		return result;
	}

	@Override
	public Object[] getMemberValues(final Object object) {
		final ExtEvent entity = (ExtEvent)object;
		final Object[] result = new Object[6];
		int index = 0;
		result[index++] = entity.getTypeFlag();
		result[index++] = entity.getVersionStamp();
		result[index++] = entity.getInitialised().getDbValue();
		result[index++] = entity.getEvent().getKey().getDbValue();
		result[index++] = entity.getExtraDataMap().getExtFixed("AGNTWAMT").getDbValue();
		result[index++] = entity.getExtraDataMap().getExtFixed("AGNTAMT").getDbValue();
		return result;
	}

	@Override
	public Object[] getMemberValuesForMember(final String memberName, final Object object) {
		if (memberName.equalsIgnoreCase("event")) {
			final Key_event member = (Key_event)object;
			final Object[] result = new Object[1];
			int index = 0;
			result[index++] = member.getKey().getDbValue();
			return result;
		}
		else if (memberName.equalsIgnoreCase("extraDataMap")) {
			final ExtraDataMap member = (ExtraDataMap)object;
			final Object[] result = new Object[2];
			int index = 0;
			result[index++] = member.getExtFixed("AGNTWAMT").getDbValue();
			result[index++] = member.getExtFixed("AGNTAMT").getDbValue();
			return result;
		}
		else {
			throw new IllegalArgumentException("Invalid memberName " + memberName);
		}
	}

	@Override
	public boolean requiresOptimisticLocking() {
		return true;
	}

	@Override
	public int getTypeFlag() {
		return typeFlag;
	}

	@Override
	public int[] getTypeFlags() {
		return typeFlags;
	}

	public static final int typeFlag =  1874;

	private static final int[] typeFlags =  { typeFlag,  };

	@Override
	public Object[] extractKeyValues(ResultSet resultSet) throws SQLException {
		return super.extractKeyValues(resultSet);
	}

	@Override
	public int bindKeyValues(Object[] keyValues, PreparedStatement stmt, final int startIndex) throws SQLException {
		return super.bindKeyValues(keyValues, stmt, startIndex);
	}

    @Override
	public Object getKeyFromEntity(final Object entity) {
		return (((ExtEvent)entity).get$key());
	}

}
